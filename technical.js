...
function filterMarket(filter) {
  ...
  $('#market-count').textContent = `${count} evidence ${count===1?'entry':'entries'} shown · maturity and integration requirements listed for each`;
}
...
const scales={
  assembly:{label:'ASSEMBLY SCALE',category:'SYSTEM INTEGRATION',title:'Small displays give us a starting point.',copy:'Microdisplays concentrate pixels into a small active area. The integration task is to combine that light source with optics, drivers, interconnects and protective barriers in a lens-compatible geometry.',evidence:'JBD lists a 3.3 mm active diagonal for an in-production microdisplay. This is an example of display miniaturization; a contact-specific optical engine needs custom development.',source:'jbd'},
  routing:{label:'FINE-SCALE INTERCONNECTS',category:'MATERIALS RESEARCH',title:'Transparent routes for electrical signals.',copy:'Flexible conductors can connect electronic islands while limiting obstruction and mechanical stress. This gives lens designers a route to distributing electronics across a curved, transparent surface.',evidence:'A 2017 contact-lens sensor study used graphene and approximately 30 nm silver nanowires. That supports a materials approach, with reliability and human-use qualification still required for a display product.',source:'nano'},
